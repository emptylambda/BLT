...
typeCheckProgram p = case runState (runExceptT (checkProgram p)) emptyContext of
...
exprType c expr = case evalState (runExceptT (checkExpression expr)) c of
...
enterProcedure sig def actuals lhss c = localContext inst (zip ins (psigArgTypes sig) ++ zip outs (psigRetTypes sig) ++ map noWhere localVars) c
  where
    inst = case evalState (runExceptT (pInstance sig actuals lhss)) c of
...
data Context = Context
  {
    ...
    procCallGraphs :: Map Id [Id],
    currentProc :: Id,
    havocs :: Map Id Type,
    typeAnnotatedProgram :: Program, 
      -- Local:
...
setCurrentProc n c  = c { currentProc = n }
...
locally check = do
  c <- get
  res <- check `catchE` (\err -> restore c >> throwE err)
  restore c
  return res
  where
    restore c = do
...
type Typing a = ExceptT [TypeError] (State Context) a
...
checkExpression (Pos pos e) = do
  ...
  case e of
    ...
    Quantified t qop tv vars e -> checkQuantified qop tv vars e
...
checkProcSignature name tv args rets specs = do
  ...
  where
    ...
    addPSig name sig c = c { ctxProcedures = M.insert name sig (ctxProcedures c),
                             procCallGraphs = M.insert name [] (procCallGraphs c)
                           }
...
-- | Check interprocedual call dependencies
checkCallDep :: Decl -> Typing ()
checkCallDep (Pos pos d) = do
  case d of
    ProcedureDecl name tv args rets specs mb -> do
      modify $ setCurrentProc name
      case mb of
        Nothing -> return ()
        Just body -> findCallInBody body
    ImplementationDecl name tv args rets bodies ->  do
      modify $ setCurrentProc name
      return ()
    otherwise -> return ()
  
findCallInBody :: Body -> Typing ()
findCallInBody body = do
  mapM_ (findInSt findCalls) body

findInSt f = mapM_ (\(Pos _ (_, st)) -> f st )

findCalls :: Statement -> Typing ()
findCalls (Pos _ st) = case st of
  Call _ callee _ -> do
    caller <- gets currentProc
    modify $ \ctx -> ctx { procCallGraphs = M.adjust ([callee] ++) caller (procCallGraphs ctx) }
  _ -> return ()
...
checkProgram (Program decls) = do
  ...
  mapAccum_ checkCallDep decls
  -- mapAccum_ checkHavocs decls
...
checkLefts vars n = if length vars /= n 
  then throwTypeError (text "Expected" <+> int n <+> text "left-hand sides and got" <+> int (length vars))
    ...
    then throwTypeError (text "Variable occurs more than once among left-handes of a parallel assignment")
      ...
      mv <- gets $ mutableVars
      let immutableLhss = vars \\ (M.keys mv)
...
checkHavocLefts :: [Id] -> Int -> Typing ()
checkHavocLefts vars n = if length vars /= n 
  then throwTypeError (text "Expected" <+> int n <+> text "left-hand sides and got" <+> int (length vars))
  else if vars /= nub vars
    then throwTypeError (text "Variable occurs more than once among left-handes of a parallel assignment")
    else do
      mv <- gets $ mutableVars
      let immutableLhss = vars \\ (M.keys mv)
          typs = M.fromList [ (id, mv M.! id) | id <- vars ]
      if not (null immutableLhss)
        then throwTypeError (text "Assignment to immutable variable(s):" <+> commaSep (map text immutableLhss))
        else do
          clocs <- gets $ M.keys . ctxLocals
          ...
          if not (null invalidGlobals)
            then throwTypeError (text "Assignment to a global variable that is not in the enclosing procedure's modifies clause:" <+> commaSep (map text invalidGlobals))
              ...
              modify $ \ctx -> ctx { havocs = M.union typs (havocs ctx) }
              return ()
