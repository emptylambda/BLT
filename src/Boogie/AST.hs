...
data BareExpression = 
  ...
  Quantified Bool QOp [Id] [IdType] Expression         -- ^ 'Quantified' @qop triggered type_vars bound_vars expr@
...
-- | Statement
data BareStatement =
  Predicate [Attribute] SpecClause |                      -- ^ Predicate statement (assume or assert)
  ...
  CallForall Id [WildcardExpression] |                    -- ^ 'CallForall' @proc_name args@ -- Depreciated 
...
type Body = ([[IdTypeWhere]], Block) -- (locals, statements)
...
data Contract =
  Requires Bool Expression |                -- ^ 'Requires' @e free@
...
-- | 7 kinds of Top-level declaration
data BareDecl = 
  TypeDecl [NewType] |
...
-- | Get rid of all positions 
stripToStatement :: LStatement -> BareStatement
stripToStatement = node . snd . node

bareSt2Block :: BareStatement -> Block
bareSt2Block = (:[]) . gen . (,) [] . gen
