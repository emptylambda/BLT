...
exprDocAt n (Pos _ e) = condParens (n' <= n) (
  case e of
    ...
    Quantified t qOp fv vars e -> t' <+>
      parens (pretty qOp <+> typeArgsDoc fv <+> commaSep (map idpretty vars) <+> text "::" <+> exprDoc e)
      -- where t' = "<<" <+> text (show t) <+> ">>"
      where t' = ""
  )
  where
...
-- statementDoc (Pos pos s) = text (show pos) <> statementDoc'(Pos pos s)
...
{- Specs -}     
-- [JEFF] Non-exhaustive here, not sure why?
specpretty :: SpecType -> Doc
